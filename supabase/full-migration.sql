-- ============================================================
-- BudaBook — COMPLETE DATABASE MIGRATION
-- Run this entire file in the Supabase Dashboard SQL Editor
-- Supabase Dashboard > SQL Editor > New Query > Paste > Run
-- ============================================================
-- After running this, also run the admin-user-seed.sql file
-- (that one needs the bcrypt hash generated by the app)
-- ============================================================
...
CREATE POLICY "accom_types_public_read" ON accommodation_types FOR SELECT USING (is_active = true);
...
CREATE POLICY "sections_public_read" ON website_sections FOR SELECT USING (is_visible = true);
CREATE POLICY "testimonials_public_read" ON testimonials FOR SELECT USING (is_featured = true);
CREATE POLICY "gallery_public_read" ON gallery_images FOR SELECT USING (is_active = true);
CREATE POLICY "rate_adj_public_read" ON rate_adjustments FOR SELECT USING (is_active = true);
CREATE POLICY "bookings_public_read_own" ON bookings FOR SELECT USING (true);
CREATE POLICY "day_tour_public_read" ON day_tour_bookings FOR SELECT USING (true);
CREATE POLICY "bookings_public_insert" ON bookings FOR INSERT WITH CHECK (true);
CREATE POLICY "day_tour_public_insert" ON day_tour_bookings FOR INSERT WITH CHECK (true);
CREATE POLICY "booking_addons_public_insert" ON booking_addons FOR INSERT WITH CHECK (true);
CREATE POLICY "dt_booking_addons_public_insert" ON day_tour_booking_addons FOR INSERT WITH CHECK (true);
CREATE POLICY "status_log_public_insert" ON booking_status_log FOR INSERT WITH CHECK (true);
CREATE POLICY "status_log_public_read" ON booking_status_log FOR SELECT USING (true);
...
CREATE POLICY "day_tour_public_update" ON day_tour_bookings FOR UPDATE USING (true) WITH CHECK (true);
...
DROP TRIGGER IF EXISTS set_updated_at_accommodation_types ON accommodation_types;
...
DROP TRIGGER IF EXISTS set_updated_at_day_tour_bookings ON day_tour_bookings;
...
DROP TRIGGER IF EXISTS set_updated_at_addons ON addons;
CREATE TRIGGER set_updated_at_addons BEFORE UPDATE ON addons FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS set_updated_at_rate_adjustments ON rate_adjustments;
...
DROP TRIGGER IF EXISTS set_updated_at_website_sections ON website_sections;
...
('a1b2c3d4-0001-0001-0001-000000000001', 'a1b2c3d4-e5f6-7890-abcd-000000000001', 'Cocoon Suite', 'cocoon-suite', 'Our newest luxury accommodation featuring a private swimming pool, hot tub, and fire pit.', 'Private pool, hot tub, and fire pit with luxury modern interiors', 19800.00, 19800.00, 4, 6, 2500.00, NULL, '["AC", "Private Swimming Pool", "Hot Tub", "Fire Pit", "En-suite Bathroom", "Modern Interiors", "Lush Forest View", "Electricity", "Hot Water"]', '["Breakfast for 4", "Welcome Grazing Platter", "Strawberry Juice", "Pool Access", "Toiletries", "Drinking Water"]', true, 1),
('a1b2c3d4-0001-0001-0001-000000000002', 'a1b2c3d4-e5f6-7890-abcd-000000000001', 'Geodesic Dome Tent', 'dome-tent', 'Our signature 28 sqm geodesic dome tents combine the thrill of camping with luxury living.', '28 sqm luxury dome with AC, Jacuzzi, Netflix, and mountain views', 9800.00, 10800.00, 4, 6, 1800.00, 28.0, '["AC", "En-suite Bathroom", "Android TV with Netflix", "Outdoor Jacuzzi", "Private Deck", "Mountain View", "Electricity", "Hot Water", "Cozy Beds", "Living Area"]', '["Breakfast for 4", "Welcome Grazing Platter", "Strawberry Juice", "Pool Access", "2-Hour Jacuzzi Bubbles", "Toiletries", "Drinking Water"]', true, 2),
('a1b2c3d4-0001-0001-0001-000000000003', 'a1b2c3d4-e5f6-7890-abcd-000000000001', 'Premium Tent', 'premium-tent', 'An elevated glamping experience with larger space and premium furnishings.', 'Upgraded dome experience with premium furnishings and superior views', 13800.00, 13800.00, 4, 8, 2000.00, NULL, '["AC", "En-suite Bathroom", "Android TV with Netflix", "Outdoor Jacuzzi", "Private Deck", "Panoramic Mountain View", "Electricity", "Hot Water", "Premium Beds", "Spacious Living Area"]', '["Breakfast for 4", "Welcome Grazing Platter", "Strawberry Wine", "Pool Access", "2-Hour Jacuzzi Bubbles", "Premium Toiletries", "Drinking Water"]', true, 3),
('a1b2c3d4-0001-0001-0001-000000000004', 'a1b2c3d4-e5f6-7890-abcd-000000000001', 'Tiny House', 'tiny-house', 'Compact and modern, our Tiny Houses offer a cozy retreat for couples or small groups.', 'Compact modern stay for couples with essential comforts', 7500.00, 7500.00, 2, 4, 1500.00, NULL, '["AC", "Private Bathroom", "Electricity", "Hot Water", "Comfortable Bed", "Small Deck"]', '["Breakfast for 2", "Welcome Juice", "Pool Access", "Toiletries", "Drinking Water"]', true, 4)
...
('a1b2c3d4-e5f6-7890-abcd-000000000001', 'Maria Santos', 5, 'Absolutely unforgettable! The dome tent was luxurious, the views were breathtaking, and the strawberry picking was such a unique experience.', 'Facebook', true),
('a1b2c3d4-e5f6-7890-abcd-000000000001', 'Carlos Reyes', 5, 'The sea of clouds at sunrise was magical. The infinity pool with mountain views is a dream. Will definitely come back!', 'Google', true),
('a1b2c3d4-e5f6-7890-abcd-000000000001', 'Ana Lim', 4, 'Beautiful place with amazing ambiance. The cocoon suite was worth every peso — private pool and hot tub made it so exclusive.', 'TripAdvisor', true),
('a1b2c3d4-e5f6-7890-abcd-000000000001', 'Jake Torres', 5, 'Perfect romantic getaway! My partner and I stayed in the Tiny House and it was cozy and charming. The bonfire under the stars was the highlight.', 'Facebook', true),
('a1b2c3d4-e5f6-7890-abcd-000000000001', 'Diane Cruz', 4, 'Great for families! Kids loved picking strawberries and swimming in the infinity pool. Highly recommended for a weekend escape.', 'Google', true)
ON CONFLICT DO NOTHING;
