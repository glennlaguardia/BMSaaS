...
function AnimatedStat({ value, label, isVisible }: { value: string; label: string; isVisible: boolean }) {
    ...
    useEffect(() => {
        ...
        const animate = (currentTime: number) => {
            ...
            // Don't add comma separators for year-like values (4-digit numbers ≤ current year)
            const isYear = targetNum >= 1000 && targetNum <= new Date().getFullYear() + 10 && suffix === '';
            setDisplayValue(`${isYear ? current.toString() : current.toLocaleString()}${suffix}`);
